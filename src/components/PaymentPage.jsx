...
const PaymentPage = () => {
  ...
  const validateForm = () => {
    const { name, location, mobile, notes, fullAddress } = address;
    if (!name || !location || !mobile || !notes || !fullAddress) {
  ...
  const handleOrder = () => {
    ...
    navigate("/delivery", { state: { productImages: orderImages } });
  };

  return (
    <div className="mt-28 md:mt-28 lg:mt-28 bg-[#FFF8F0] text-gray-800 min-h-screen p-4 sm:p-8 space-y-10">
              ...
              <div className="mt-10">
                ...
                <div>
                  ...
                  <div className="flex items-center border border-gray-300 rounded w-28">
                    <button
                      onClick={() =>
                        handleQuantityChange(item.id, item.selectedWeight, (item.quantity || 1) - 1)
                      }
